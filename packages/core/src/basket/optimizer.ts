...
function buildSingleRetailerPlan(
  ...
  return {
    ...
    grandTotal: subtotal + quote.fee + quote.travelCost + quote.timeCost - quote.loyaltyRebate,
...
function buildMultiRetailerPlan(
  ...
  return {
    ...
    totalTimeCost,
    totalLoyaltyRebate,
    grandTotal: subtotal + totalFees + totalTravelCost + totalTimeCost - totalLoyaltyRebate,
    coverage: lines.length / items.length,
